...
fn strip_untrusted_provider_fields(patch: &mut toml::Table) {
    ...
    // rollout payloads. `auth_provider` is also stripped by the caller's key
    // list, but removing it here keeps this trust boundary self-contained.
...
mod tests {
    ...
    fn apply_patches_strips_the_top_level_keys() {
        ...
        // Provider selection is stripped even when nested below a model; legacy
        // non-authoritative metadata remains available to the ordinary merge.
        ...
        assert!(cfg3["model"]["x"].get("auth_provider").is_none());
        assert_eq!(
    ...
    #[test]
    fn higher_layer_patch_cannot_combine_credentials_with_lower_layer_helper() {

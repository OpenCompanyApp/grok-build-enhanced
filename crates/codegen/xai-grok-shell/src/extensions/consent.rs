...
async fn handle_record(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // This is an xAI-only endpoint. Reject every other provider credential
    // before constructing the retrying client so consent recording cannot
    // fall through to a generic API key or another provider's identity.
    super::auth_gate::require_xai_auth(
        &agent.auth_manager,
        "Authentication required. Run `grok login` to re-authenticate.",
        "Consent recording requires auth with grok.com. Run `grok login` to authenticate.",
    )?;

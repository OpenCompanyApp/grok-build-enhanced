...
fn local_resume_credential_binding(
    ...
    let summary = crate::session::persistence::find_summary_by_session_id(&source.child_session_id)
...
pub(crate) async fn run_shell_child(
    ...
    if let Some(error) = task_model_override_error(
        ...
        ctx.auth_manager
            .current_or_expired()
            .is_some_and(|a| a.auth_mode.is_session_auth()),
    ) {
    ...
    let source_binding = match resume_source.as_ref() {
        Some(source) => {
            match local_resume_credential_binding(source, effective_sampling_config.provider) {
                Ok(binding) => binding,
                Err(reason) => {
                    let message = format!(
                    ...
                    return child_run_output(
                        failure_result(&request, &message),
                        completion_data,
                        None,
                    );
                }
    ...
    let generic_api_key_provider = (effective_sampling_config.provider
    ...
    .await
    {
        Ok(runtime) => runtime,
        Err(error) => {
            let message = format!("Provider binding failed: {error}");
    ...
    *tool_ctx.codex_root_turn_id.lock() = ctx.codex_root_turn_id.clone();
    ...
    xai_grok_telemetry::unified_log::info(
        ...
        Some(serde_json::json!({
            ...
            "credential_present": has_api_key(&effective_sampling_config.api_key),
            ...
            "parent_credential_present": has_api_key(&ctx.sampling_config.api_key),
    ...
    let _ = persistence
        ...
        .send(crate::session::persistence::PersistenceMsg::CurrentModel {
            ...
            comp_hash: Some(effective_sampling_config.comp_hash.clone()),
            credential_binding: effective_sampling_config.credential_binding.clone(),
        });
    let spawn_result = session::spawn_session_on_thread(
        ...
        ctx.codex_web_search_settings.clone(),

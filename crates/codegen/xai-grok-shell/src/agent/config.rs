...
/// `[shell_environment_policy]` known keys, declared for the unrecognized-key
/// scan only. The policy is parsed at process-spawn time by
/// `xai_grok_tools::util::resolve_shell_env_policy`; keeping the values opaque
/// here makes wrong types and typos warnings instead of config-load failures.
#[derive(Clone, Debug, Default, Deserialize)]
#[serde(default)]
...
pub struct Config {
    ...
    /// Validation only; the value is parsed at spawn by
    /// `xai_grok_tools::util::resolve_shell_env_policy`.
...
impl Config {
    ...
    fn resolve_subagent_limits(
        ...
        self.subagents_max_concurrent = SubagentsConfig::resolve_max_concurrent(
            ...
            sa.max_concurrent,
            remote.and_then(|settings| settings.subagents_max_concurrent),
        );
        self.subagents_limit_behavior = SubagentsConfig::resolve_limit_behavior(
            ...
            remote.and_then(|settings| settings.subagents_limit_behavior.as_deref()),
        );
        self.workflow_max_concurrent_agents = SubagentsConfig::resolve_workflow_max_concurrent(
            ...
            remote.and_then(|settings| settings.workflow_max_concurrent_agents),
        );
    }
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        let subagents_toml = crate::config::SubagentsConfig {
            max_concurrent: ctx
                .raw_config
                .get("subagents")
                .and_then(|section| section.get("max_concurrent"))
                .and_then(toml::Value::as_integer),
            limit_behavior: ctx
                .raw_config
                .get("subagents")
                .and_then(|section| section.get("limit_behavior"))
                .and_then(toml::Value::as_str)
                .map(str::to_owned),
            workflow_max_concurrent: ctx
                .raw_config
                .get("subagents")
                .and_then(|section| section.get("workflow_max_concurrent"))
                .and_then(toml::Value::as_integer),
            ..Default::default()
        };
    ...
    /// Per-turn dashboard summary gate. Default ON.
...
pub struct Features {
    ...
    /// Per-turn dashboard summary generated after a successful turn.
    #[serde(default, skip_serializing_if = "Option::is_none")]

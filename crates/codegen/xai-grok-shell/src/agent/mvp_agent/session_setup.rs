...
use super::acp_agent::{
    ...
    resolve_persisted_openai_codex_model_for_load, restored_codex_comp_hash,
};
...
impl MvpAgent {
    ...
    async fn attach_session(
        ...
        restored_sampling.credential_binding = (
            ...
            .flatten();
        let (restored_summary_client, restored_summary_model) =
            self.build_summary_client(&restored_sampling).await?;
        persistence
            .replace_summary_runtime(
                restored_summary_client,
                restored_summary_model,
        ...
        if !self.is_resident(&session_id) {
            ...
            let persisted_agent_name: Option<String> = summary.agent_name.clone().or_else(|| {
                self.resolve_model_id(&spawn_model_id)
                    .ok()
    ...
    async fn restore_persisted_model(
        ...
        session_id: &acp::SessionId,
        summary: &crate::session::persistence::Summary,
        persisted_provider_model: Option<(

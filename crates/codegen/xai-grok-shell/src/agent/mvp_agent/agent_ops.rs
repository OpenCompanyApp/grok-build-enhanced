...
impl MvpAgent {
    ...
    pub(crate) fn command_availability(
        ...
        crate::session::slash_commands::CommandAvailability {
            ...
            fast: self.cfg.borrow().features.fast_mode.unwrap_or(true),
    ...
    /// Product analytics follows the telemetry switch and team ZDR policy;
    /// coding-data retention is a separate control.
    ...
    pub(super) fn prepare_web_fetch_config(
        ...
        let enabled = cfg.feature(crate::agent::config::Feature::WebFetch);
    ...
    /// right behind it must wait for the session to become resident in the registry

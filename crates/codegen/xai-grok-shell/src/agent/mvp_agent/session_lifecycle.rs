...
impl MvpAgent {
    ...
    /// Remove a session and every session-scoped registry entry **without**
    /// finalizing the cloud replica.
    ///
    ...
    /// terminal closes go through [`MvpAgent::close_session_explicit`].
    /// Completed subagent results are deliberately retained by their bounded
    /// coordinator cache so polling and `resume_from` keep working after close.
    ...
    /// plus the workspace binding and bounded subagent lifecycle maps.
    pub(crate) fn registry_snapshot(&self) -> RegistrySnapshot {
        ...
        RegistrySnapshot {
            ...
            workspace_bindings: self
                ...
                .borrow()
                .as_ref()
                .and_then(|ops| ops.workspace_handle().map(|handle| handle.session_count())),
        }
    }
}

...
pub(crate) struct SessionSpawnOptions<'a> {
    ...
    pub restored_credential_binding: Option<xai_grok_sampling_types::CredentialBinding>,
    pub restored_comp_hash: Option<String>,
...
pub struct MvpAgent {
    ...
    /// Single-flight guard for interactive login attempts.
...
struct AuthRequestMeta {
    ...
    /// Pager auth request sequence used to scope cancellation.
    #[serde(default, rename = "requestSeq")]
...
impl MvpAgent {
    ...
    pub(crate) async fn retry_subscription_check(&self) {
        ...
        if let Some(unblocked) = result {
            ...
            let claim_already_current = self
                ...
                .is_some_and(|claim| {
                    jwt_claim_matches_user_subscription_tier(&claim, &unblocked.new_tier)
                });
            let refresh_ok = if claim_already_current {
                true
            } else if unblocked.refresh_deadline_hit {
                tracing::info!(
                    "post-unblock: skipping forced mint, bounded refresh remains in flight"
                );
                xai_grok_telemetry::unified_log::info(
    ...
    fn tier_recheck_identity_changed(
        ...
        xai_grok_telemetry::unified_log::info(
            ...
            None,
            Some(serde_json::json!({
                "started_user_id_present": !started_user_id.is_empty(),
                "canonical_user_id_present": canonical_user_id.is_some_and(|id| !id.is_empty()),
                "live_user_id_present": live.as_deref().is_some_and(|id| !id.is_empty()),
            })),
        );
        true
    }

...
fn undefined_auth_provider_fails_closed() {
    ...
    assert!(cfg.config_warnings.iter().any(|warning| {
        warning.kind == super::super::config_model_override_parse::ConfigWarningKind::InvalidValue
            && matches!(
                &warning.target,
...
fn acp_non_codex_model_meta_omits_codex_service_tier_state() {
    let mut models = IndexMap::new();
    let mut entry = test_model_entry("custom-luna", "https://example.test/v1", None, None, None);
    entry.info.provider = ProviderId::Custom;
    entry.info.service_tier = Some("priority".to_string());
...
fn resolve_doom_loop_recovery_precedence() {
    ...
    assert_eq!(
        default_policy.max_threshold, 64,
        "default tunables unchanged"
    );
    assert_eq!(default_policy.max_retries, 2);
    assert_eq!(
        default_policy.window_tokens, 1024,
        "default tunables unchanged"
    );

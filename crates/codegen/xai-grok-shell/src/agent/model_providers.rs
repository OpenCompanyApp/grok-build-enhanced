...
mod tests {
    ...
    fn model_own_unresolved_key_ignores_provider_inline_auth() {
        ...
        assert!(
            model.effective_auth_provider().is_none(),
            "the provider's live inline auth must not become effective"
        );
        assert_eq!(
            resolve_credentials(model, Some("session-jwt")).api_key,
            None,
            "the unresolved model key must ignore provider inline auth and session auth"
        );
    }
    ...
    fn fail_closed_ref_ignores_a_colliding_auth_provider_table() {
        ...
        assert!(
            !provider.is_bound_to_custom_route(&model.info.base_url),
            "the fail-closed ref must not become an effective provider"
        );
    }

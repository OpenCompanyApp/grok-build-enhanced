    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .map(|h| {
            ...
            HookEntry {
                ...
                hook_type: h.handler_type.as_str().to_owned(),
                target: h
                    .command
                    .as_ref()
...
mod tests {
    ...
    #[test]
    fn inspect_warnings_omit_malformed_auth_provider_values() {
        const COMMAND_SENTINEL: &str = "private-command-sentinel";
        const ARGS_SENTINEL: &str = "private-args-sentinel";
        let effective: toml::Value = toml::from_str(&format!(
            r#"
        ...
        let cfg = crate::agent::config::Config::new_from_toml_cfg(&effective).unwrap();
        let human = render_config_warnings(&cfg.config_warnings);
        let json = serde_json::to_string(&cfg.config_warnings).unwrap();

        assert!(human.contains("[auth_provider.\"malformed\"]"));
        assert!(json.contains("malformed"));
        for forbidden in [COMMAND_SENTINEL, ARGS_SENTINEL] {
            assert!(
                !human.contains(forbidden),
                "human inspect leaked {forbidden}"
            );
            assert!(!json.contains(forbidden), "JSON inspect leaked {forbidden}");
        }
    }

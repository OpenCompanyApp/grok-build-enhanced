...
/// Named custom-model providers intentionally accept a narrower JSON contract
/// than the legacy xAI external-auth command. In particular, an xAI `issuer`
/// or an unknown field must not silently cross into the custom-provider path.
#[derive(serde::Deserialize)]
#[serde(deny_unknown_fields)]
struct ProviderAuthOutput {
    access_token: String,
    #[serde(default)]
...
pub(crate) fn parse_token_output(
    ...
    if stdout.starts_with('{') {
        ...
        tracing::debug!(
            ...
            has_issuer = parsed.issuer.is_some(),
...
/// Parse the deliberately narrow output contract for a named custom-model
/// provider. Errors describe only the rejected shape; they never include the
/// helper's stdout, stderr, command, or arguments.
pub(crate) fn parse_provider_token_output(
    output: &std::process::Output,
) -> anyhow::Result<ParsedTokenOutput> {
    if !output.status.success() {
        anyhow::bail!("exited unsuccessfully");
    }
    let stdout = std::str::from_utf8(&output.stdout)
        .map_err(|_| anyhow::anyhow!("produced non-UTF-8 output on stdout"))?
        .trim();
    ...
    if stdout.starts_with('{') {
        let parsed: ProviderAuthOutput = serde_json::from_str(stdout)
            .map_err(|_| anyhow::anyhow!("produced JSON outside the provider token contract"))?;
        let access_token = parsed.access_token.trim().to_owned();
        if access_token.is_empty() {
            anyhow::bail!("produced JSON with an empty access_token");
        }
        ...
        let refresh_token = match parsed.refresh_token {
            Some(value) => {
                ...
                if value.is_empty() {
                    anyhow::bail!("produced JSON with an empty refresh_token");
                }
        ...
        let expires_at = match parsed.expires_in {
            Some(seconds) => Some(
                expiry_after_seconds(seconds)
                    .ok_or_else(|| anyhow::anyhow!("produced an unrepresentable expires_in"))?,
            ),
            None => None,
        };
        tracing::debug!(
            ...
            "auth: parsed named provider output as JSON"
        );
        return Ok(ParsedTokenOutput {
            access_token,
            refresh_token,
    ...
    // A JSON array/string is never a bearer token payload. Reject these shapes
    // rather than treating their serialized bytes as an opaque token.
    if stdout.starts_with('[') || stdout.starts_with('"') {
        anyhow::bail!("produced JSON outside the provider token contract");
    }
    reject_control_chars(stdout)?;
    tracing::debug!(
        stdout_len = stdout.len(),
        "auth: treating named provider output as a bare token"
    );
    Ok(ParsedTokenOutput {
        access_token: stdout.to_owned(),
        refresh_token: None,
...
mod tests {
    ...
    #[test]
    fn provider_parser_accepts_only_the_documented_json_fields() {
        let output = std::process::Output {
            status: std::process::Command::new("true").status().unwrap(),
            stdout: br#"{"access_token":" token ","refresh_token":" refresh ","expires_in":3600}"#
                .to_vec(),
            stderr: vec![],
        };

        let parsed = parse_provider_token_output(&output).unwrap();
    ...
    #[test]
    fn provider_parser_rejects_external_auth_identity_fields() {
        let output = std::process::Output {
            status: std::process::Command::new("true").status().unwrap(),
            stdout: br#"{"access_token":"token","issuer":"https://auth.x.ai"}"#.to_vec(),
            stderr: vec![],
        };

        assert!(parse_provider_token_output(&output).is_err());
    }

    #[test]
    fn provider_parser_rejects_empty_or_control_bearing_refresh_tokens() {
        let ok = |stdout: &str| std::process::Output {
            status: std::process::Command::new("true").status().unwrap(),
            stdout: stdout.as_bytes().to_vec(),
            stderr: vec![],
        ...
        assert!(
            parse_provider_token_output(&ok(r#"{"access_token":"token","refresh_token":"   "}"#))
                .is_err()
        );
        assert!(
            parse_provider_token_output(&ok(
    ...
    #[test]
    fn provider_parser_rejects_unrepresentable_expiry() {
        let output = std::process::Output {
            status: std::process::Command::new("true").status().unwrap(),
            stdout: format!(r#"{{"access_token":"token","expires_in":{}}}"#, u64::MAX).into_bytes(),
            stderr: vec![],
        };
    ...
    #[test]
    fn provider_parser_rejects_json_non_object_shapes() {
        let ok = |stdout: &str| std::process::Output {
            status: std::process::Command::new("true").status().unwrap(),
            stdout: stdout.as_bytes().to_vec(),
            stderr: vec![],
        ...
        assert!(parse_provider_token_output(&ok(r#"["token"]"#)).is_err());
        assert!(parse_provider_token_output(&ok(r#""token""#)).is_err());
    }
}

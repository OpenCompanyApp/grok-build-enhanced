...
pub struct AuthManager {
    ...
    /// This is xAI-only static-key state; provider-scoped credentials never
    /// enter this resolver.
...
impl AuthManager {
    pub fn new(grok_home: &Path, grok_com_config: GrokComConfig) -> Self {
        ...
        let path = crate::auth::resolved_auth_path(grok_home);
        ...
        let (auth, auth_read_detail, initial_disk_state) = match read_auth_json(&path) {
            Ok(map) => {
                ...
                let detail = serde_json::json!({
                    ...
                    "has_access_token": found.as_ref().map(|a| !a.key.is_empty()),
                });
    ...
    /// Cached token if still wire-valid, ignoring the early-invalidation
    /// buffer. Sync callers cannot refresh and must not demote a token the
    ...
    fn try_adopt_disk_token(&self, reason: RefreshReason, msg: &str) -> Option<GrokAuth> {
        ...
        xai_grok_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                ...
                "has_refresh_token": refreshed.refresh_token.is_some(),
            })),
        );
        Some(refreshed)
    }
                    ...
                    if sibling_rotated {
                        ...
                        xai_grok_telemetry::unified_log::info(
                            ...
                            Some(serde_json::json!({
                                ...
                                "tried_refresh_token_present": tried_refresh_token.is_some(),
                                "disk_refresh_token_present": disk_rt.is_some(),
                            })),
                        );
                        return Err(AuthError::transient_reason(
...
impl xai_grok_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
    fn current_api_key(&self) -> Option<String> {
        ...
        // Sync callers cannot refresh, so a buffered-but-valid token still
        // takes precedence over a static xAI key.
        self.0
            .current_wire_valid()
            .map(|a| a.key)
...
///
/// This is the public path for out-of-crate xAI consumers such as the pager's
/// voice channel. Provider-scoped Codex, Kimi, and custom credentials remain in
/// their isolated managers and are never consulted here.

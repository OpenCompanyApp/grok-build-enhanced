...
/// Compatibility path for callers that still require cold-start minting.
/// Readiness-sensitive callers should use [`try_noninteractive_auth_no_mint`]
/// and start the mint after their protocol endpoint is ready.
pub(crate) async fn try_ensure_session_noninteractive(
    grok_com_config: &GrokComConfig,
) -> Option<GrokAuth> {
    let auth_manager = build_startup_auth_manager(grok_com_config);
    if let Some(auth) = try_noninteractive_auth_no_mint_with(&auth_manager).await {
        return Some(auth);
    }
    mint_session_noninteractive(&auth_manager).await
}

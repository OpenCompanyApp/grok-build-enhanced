...
async fn probe_handback_environment(
    mark_expired: bool,
    ...
    command.args([
        "-c",
        "printf '%s|%s|%s|%s' \
         \"${GROK_AUTH_EXPIRED-unset}\" \
         \"${GROK_AUTH_PROVIDER_ACCESS_TOKEN-unset}\" \
         \"${GROK_AUTH_PROVIDER_REFRESH_TOKEN-unset}\" \
         \"${GROK_AUTH_PROVIDER_EXPIRES_AT-unset}\"",
    ]);
    for variable in PROVIDER_HANDBACK_ENV_VARS {
        command.env(variable, "stale-wrong-slot-value");
    }
    configure_provider_handback_env(&mut command, mark_expired, previous);
...
async fn attach_trusted_config_lets_a_revived_ref_mint() {
    ...
    revived.attach_trusted_config(Some(&template.config), Some(AuthProviderRef::TEST_ROUTE));
    assert_eq!(
...
#[tokio::test]
async fn first_mint_clears_stale_ambient_handback_values() {
    let observed = probe_handback_environment(false, None).await;

    assert_eq!(observed, "unset|unset|unset|unset");
}

#[tokio::test]
async fn provider_slot_clears_handback_values_absent_from_its_previous_token() {
    let previous = MintedProviderToken {
        token: "current-slot-access".to_owned(),
        refresh_token: None,
        minted_at: std::time::Instant::now(),
        expires_at: None,
        minted_with: AuthProviderConfig::default(),
    };

    let observed = probe_handback_environment(false, Some(&previous)).await;

    assert_eq!(observed, "unset|current-slot-access|unset|unset");
}
...
async fn provider_expiry_source_precedence() {
    ...
    fn jwt_with_exp(exp: i64) -> String {
        ...
        let payload = encoder.encode(serde_json::json!({ "exp": exp }).to_string());
...
#[tokio::test]
async fn provider_unrepresentable_expiry_fails_closed() {
    let provider = AuthProviderRef::new(
...
#[tokio::test]
async fn same_named_providers_on_different_routes_never_share_tokens() {
    let first = AuthProviderRef::new_for_test_route(
        "route-isolation".to_owned(),
        AuthProviderConfig {
            command: "printf route-one-token".to_owned(),
            args: None,
            token_ttl_secs: Some(3600),
            timeout_secs: None,
        },
    ...
    let second = AuthProviderRef::new_for_test_route(
        "route-isolation".to_owned(),
        AuthProviderConfig {
            command: "printf route-two-token".to_owned(),
            args: None,
            token_ttl_secs: Some(3600),
            timeout_secs: None,
        },
    ...
    assert_eq!(
        first.ensure_fresh_token(None).await.rotated().as_deref(),
        Some("route-one-token")
    );
    assert_eq!(
        second.ensure_fresh_token(None).await.rotated().as_deref(),
        Some("route-two-token")
    );
    assert_eq!(first.cached_token().as_deref(), Some("route-one-token"));
    assert_eq!(second.cached_token().as_deref(), Some("route-two-token"));
}

#[test]
fn provider_route_binding_is_exact_and_never_serialized() {
    let config = AuthProviderConfig {
        command: "printf token".to_owned(),
        args: None,
        token_ttl_secs: None,
        timeout_secs: None,
    };
    ...
    assert!(provider.is_bound_to_custom_route("https://gateway.example/v1"));
    assert!(!provider.is_bound_to_custom_route("https://gateway.example/v1/"));

    let bytes = serde_json::to_string(&provider).unwrap();
    assert_eq!(bytes, r#"{"name":"exact-route"}"#);
    let mut revived: AuthProviderRef = serde_json::from_str(&bytes).unwrap();
    assert!(!revived.is_bound_to_custom_route("https://gateway.example/v1"));
    revived.attach_trusted_config(Some(&config), Some("https://gateway.example/v1"));
    assert!(revived.is_bound_to_custom_route("https://gateway.example/v1"));
}

#[test]
fn provider_debug_never_exposes_helper_command_or_arguments() {
    let config = AuthProviderConfig {
        command: "sentinel-command-secret".to_owned(),

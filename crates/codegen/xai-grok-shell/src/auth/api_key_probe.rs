...
/// Construct the wire credential as a sensitive header so request diagnostics
/// redact it. No key fragment is ever copied into logs or telemetry.
fn sensitive_bearer(key: &str) -> Result<HeaderValue, InvalidHeaderValue> {
...
async fn probe_xai_api_key_at_url(key: &str, url: &str, timeout: Duration) -> ApiKeyProbeVerdict {
    ...
    let Ok(authorization) = sensitive_bearer(key) else {
        return ApiKeyProbeVerdict::Unusable;
    };
...
mod tests {
    ...
    #[test]
    fn bearer_header_is_sensitive_and_diagnostics_do_not_expose_the_key() {
        const KEY: &str = "xai-test-canary-do-not-print";
        let header = sensitive_bearer(KEY).expect("valid test header");
        assert!(header.is_sensitive());
        assert!(!format!("{header:?}").contains(KEY));
    }

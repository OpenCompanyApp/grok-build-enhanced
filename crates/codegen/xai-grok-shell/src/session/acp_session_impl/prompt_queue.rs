...
impl SessionActor {
        ...
        if policy.authority.is_human_intent() {
            ...
            if preempt_armed {
                ...
                if let Some(ref auto_wake) = self.tool_context.auto_wake_delivered {
                    for id in dropped
                        .iter()
                        .filter_map(|i| i.input_origin.completion_id())
                    {
                        auto_wake.remove(id);
                    }
                }
    ...
    fn respond_removed_queued_prompt(state: &mut State, item: InputItem) {
    ...
    pub(super) async fn handle_remove_queued_prompt(
        ...
        {
            if let Some(item) = state.pending_inputs.remove(pos) {
                Self::respond_removed_queued_prompt(&mut state, item);
            }
            removed = true;
        }

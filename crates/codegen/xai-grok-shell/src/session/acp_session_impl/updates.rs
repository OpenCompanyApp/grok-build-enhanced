    ...
    pub(super) async fn handle_xai_session_notification(
        ...
        match &notification.update {
            ...
            XaiSessionUpdate::SubagentFinished {
                ...
                // The child session itself fires the genuine, awaited
                // SubagentStop gate. The parent completion observer must not
                // emit a duplicate, non-resampling SubagentStop event.
                self.subagent_spawn_info.lock().remove(subagent_id);
                {
                    let mut records = self.subagent_token_records.lock();

...
pub(crate) enum SamplerFailureRecovery {
    ...
    CompactAndResubmit,
    /// Auth 401 recovery succeeded. Carry the wire provenance of the rejected
    /// request and the store that minted the replacement credential.
...
pub(crate) enum TurnOutcome {
    ...
    Completed {
        ...
        /// provider gave no message) and maps the ACP stop reason to `Refusal`.

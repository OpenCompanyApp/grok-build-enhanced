...
> {
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    > = if let Some(ref storage) = memory_storage_for_session {
        ...
        let embedding_auth = memory_embedding_route
            .allow_session_auth
            .then(|| {
                auth_manager.as_ref().map(|manager| {
                    std::sync::Arc::new(
            ...
            .flatten();
        let embedding_api_key = memory_embedding_route
            .allow_session_auth
            .then(|| api_key_provider.clone())
            .flatten();
        let embed_credentials = crate::session::memory::EndpointScopedCredentials::for_endpoint(
            &memory_embedding_route.base_url,
            |_| memory_embedding_route.allow_session_auth,
            embedding_auth,
            embedding_api_key,
        );
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        last_recap_main_turn: std::cell::Cell::new(
            crate::session::helpers::session_recap::load_recap_watermark(
                &crate::session::persistence::session_dir(&session_info),
            ),
        ),

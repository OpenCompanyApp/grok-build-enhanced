...
impl JsonlStorageAdapter {
    ...
    pub(crate) fn copy_session_data_sync(
        ...
        // These mirrors are outside the persistence actor. Carry both text
        // and its origin so resumed/forked sessions preserve explicit client
        // instructions while model-owned prompts remain eligible to rebuild.
        for name in [
            crate::session::SYSTEM_PROMPT_FILENAME,
            crate::session::SYSTEM_PROMPT_PROVENANCE_FILENAME,
        ] {
            let _ = copy_sidecar_file(
                true,
                &self.session_dir(source_info).join(name),
...
fn fork_summary(
    ...
    // Provider-qualified sessions carry a non-secret binding to the local
    // credential record that owns them. Preserve it only when the fork stays
    // on the same explicit provider route; a model/provider switch must fail
    // closed instead of inheriting another provider's credentials.
    let provider_for_model_id = |model_id: &acp::ModelId| {
        ...
        } else if model_id.0.starts_with("opencode-go/") {
            Some(xai_grok_sampling_types::ProviderId::OpenCodeGo)
        } else {
            None
        }
    };
    let source_provider = provider_for_model_id(&source.current_model_id);
    ...
    let credential_binding = source
        .credential_binding
        .filter(|binding| Some(binding.provider) == target_provider);
    let comp_hash = (source_provider == Some(xai_grok_sampling_types::ProviderId::OpenAiCodex)
        && target_provider == source_provider)
        .then_some(source.comp_hash)
        .flatten();

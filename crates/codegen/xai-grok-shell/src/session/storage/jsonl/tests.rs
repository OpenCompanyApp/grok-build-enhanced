...
#[tokio::test]
async fn checkpoint_record_with_non_checkpoint_path_is_rejected() {
    let temp_dir = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    let source_info = Info {
        id: acp::SessionId::new("ckpt-src"),
    ...
    let result = adapter
        .copy_session_data(&source_info, &target_info, CopySessionOptions::default())
        .await
        .expect("copy preserves opaque updates while skipping unsafe checkpoint paths");
    assert_eq!(result.compaction_checkpoints_copied, 0);
    let error = adapter
        .load_session(&target_info)
        .await
        .expect_err("loading a noncanonical checkpoint reference must fail closed");
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidData);
    assert!(
        error
            .to_string()
            .contains("checkpoint file does not match its canonical session path")
    );
}
#[cfg(unix)]
#[tokio::test]

...
async fn checkpoint_record_with_non_checkpoint_path_is_not_copied() {
    ...
    // Read only the copied update stream here. A full session load correctly
    // rejects the doctored checkpoint metadata while rebuilding chat history;
    // this assertion is specifically that the copy did not overwrite the
    // rewritten stream with the referenced source path.
    let updates = adapter

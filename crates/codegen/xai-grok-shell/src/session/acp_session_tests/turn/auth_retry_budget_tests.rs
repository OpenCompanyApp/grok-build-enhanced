...
fn fail_closed_401_is_uncharged_and_turn_survives() {
    std::thread::Builder::new()
        .name("auth-retry-fail-closed-test".to_owned())
        .stack_size(16 * 1024 * 1024)
        .spawn(|| {
            let runtime = tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("build fail-closed auth retry test runtime");
            runtime.block_on(fail_closed_401_is_uncharged_and_turn_survives_impl());
        })
        .expect("spawn fail-closed auth retry test thread")
        .join()
        .expect("fail-closed auth retry test thread panicked");
}
...
/// generic budget message. Paused time auto-advances the backoff ladder. The
/// Enhanced multi-provider turn future needs the same explicit large-stack
/// harness used by the other full-turn regression tests.
#[test]
fn authenticated_401s_still_exhaust_after_three_retries() {
    std::thread::Builder::new()
        .name("auth-retry-budget-test".to_owned())
        .stack_size(16 * 1024 * 1024)
        .spawn(|| {
            let runtime = tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("build auth retry test runtime");
            runtime.block_on(async {
                ...
                local
                    .run_until(async {
                        ...
                        let err =
                            outcome.expect_err("authenticated 401s must exhaust and fail the turn");
                        let rendered = serde_json::to_string(&err.data).unwrap_or_default();
                        assert!(
                            rendered
                                .contains("authenticated inference requests were still rejected"),
        ...
        .expect("spawn large-stack auth retry test thread")
        .join()
        .expect("auth retry test thread");
}

        ...
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    ...
                    assert!(
                        artifact.x_grok_conv_id.starts_with("recap-"),
                        "recap conversation id: {}",
                        artifact.x_grok_conv_id
                    );
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            cfg.base_url = server.url();
            cfg.api_backend = xai_grok_sampling_types::ApiBackend::Responses;
            cfg.provider = xai_grok_sampling_types::ProviderId::Custom;
            actor.chat_state_handle.update_sampling_config(cfg);
            ...
            assert!(
                ...
                "the local custom-origin harness must not receive xAI conversation headers"
            );

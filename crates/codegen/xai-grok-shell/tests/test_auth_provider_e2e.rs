...
async fn provider_backed_model_sends_minted_token_on_the_wire() {
    ...
    std::fs::write(
        &helper,
        format!(
            "#!/bin/sh\necho run >> {}\nprintf 'helper-private-stderr' >&2\nprintf 'gateway-tok-1'\n",
            counter.display()
        ),
    )
...
/// A custom model that references an undefined `[auth_provider.<name>]` must
/// fail closed before inference: no unauthenticated request is attempted and
/// the session credential is never substituted onto the wire.
#[tokio::test]
#[ignore]
...
"#,
    ...
    assert!(
        ...
        "an undefined helper must abort before inference; requests:\n{}",
        server.request_log_summary()
    );
    assert!(
        !requests.iter().any(|e| {

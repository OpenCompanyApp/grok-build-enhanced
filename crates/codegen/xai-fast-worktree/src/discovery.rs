...
/// The filesystem scanner and disk-usage bucketing must agree on this.
...
impl DiscoveredWorktree {
    pub fn into_record(self) -> WorktreeRecord {
        ...
        WorktreeRecord {
            ...
            path,
            source_repo: dunce::canonicalize(&source_repo).unwrap_or(source_repo),
            repo_name,
            kind: self.kind,
...
pub fn rebuild_worktree_db(
    ...
    for mut wt in discovery.found {
        ...
        // Persist the canonical identity used for containment and duplicate
        // checks, including a standalone worktree's self source repository.
        wt.path = path;
        wt.source_repo = wt
            .source_repo
            .map(|source| dunce::canonicalize(&source).unwrap_or(source));

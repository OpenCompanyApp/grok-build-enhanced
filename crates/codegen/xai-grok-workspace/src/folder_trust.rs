...
/// Whether any repo-local trust-sensitive config is present for `cwd`. When
/// none are present there is nothing to gate, so we skip the prompt entirely.
///
...
/// `cwd` (`mcp`, `plugins`, `lsp`, `envrc`, `claude`, `hooks`, `agents`,
/// `roles`, `personas`), deduped in cheap→expensive marker order. Single source
/// with [`repo_configs_present`] (which is `!repo_config_kinds(cwd).is_empty()`),
/// so a folder that the gate fired on always has a non-empty, accurate kind list.
/// NOT itself the trust gate.
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    // Roles and personas are loaded from the exact session cwd. Treat unreadable
    // paths as present so permission errors cannot silently bypass the trust gate.
...
mod tests {
    ...
    #[test]
    fn project_role_marker_regular_file_is_not_a_directory() {
        let tmp = repo_tmp();
        let grok = tmp.path().join(".grok");
        std::fs::create_dir_all(&grok).unwrap();
    ...
    #[test]
    fn project_role_marker_at_repo_root_is_absent_from_subdir() {
        let tmp = repo_tmp();
    ...
    #[cfg(unix)]
    #[test]
    fn project_role_marker_symlink_to_directory_is_present() {
        let tmp = repo_tmp();
    ...
    #[cfg(unix)]
    #[test]
    fn dangling_project_persona_marker_is_absent() {
        let tmp = repo_tmp();
        let grok = tmp.path().join(".grok");
        std::fs::create_dir_all(&grok).unwrap();

...
impl PermissionHandle {
    ...
    /// Replace the provider-resolved static web-fetch allowlist.
...
fn spawn_permission_manager_with_pin(
    ...
    let _task = tokio::task::spawn_local(async move {
        ...
        let mut web_fetch_allowlist_is_default = web_fetch_allowed_domains
            .iter()
            .map(String::as_str)
        ...
        while let Some(cmd) = rx.recv().await {
            match cmd {
                ...
                PermissionCommand::SetWebFetchAllowedDomains(domains) => {
                    web_fetch_allowlist_is_default = domains
                        .iter()
                        .map(String::as_str)
                        .eq(DEFAULT_ALLOWED_DOMAINS.iter().copied());

//! Subprocess-based integration tests for inherited npm helper functions.
//!
//! `auto_update::install_npm_for_test` and `version::fetch_npm_tag_for_test`
//! spawn `npm` by bare name (`Command::new("npm")`). To test these explicitly
//! non-production helpers without touching a registry, we install a
//! tempdir-resident shell script that logs its args and prints canned stdout.
//!
//! Enhanced GitHub Release discovery does not shell out to `gh`; its public
//! REST contract is covered hermetically by WireMock in the fork-routing and
//! check-status suites.
//!
...
// Fork release discovery is intentionally absent from this subprocess suite:
// production uses the fork-owned REST API and never invokes the `gh` CLI.

...
async fn a_log_at_the_budget_is_complete() {
    ...
    assert_eq!(read_prefix(&path, 64).await, (body, false));
}
...
async fn stops_at_the_budget_and_reports_more() {
    ...
    assert_eq!(read_prefix(&path, 500).await, ("X".repeat(500), true));
}
...
async fn drops_a_character_split_by_the_budget() {
    ...
    assert_eq!(read_prefix(&path, 8).await, ("日日".to_string(), true));
}
...
async fn a_log_ending_mid_character_reads_as_incomplete() {
    ...
    assert_eq!(read_prefix(&path, 100).await, (String::new(), true));
}

#[tokio::test]
async fn a_missing_log_reads_as_incomplete() {
    assert_eq!(
        read_prefix(Path::new("/nonexistent/task.log"), 100).await,
        (String::new(), true)
    );
}

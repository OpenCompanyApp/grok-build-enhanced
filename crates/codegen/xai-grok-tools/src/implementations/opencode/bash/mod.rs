...
mod tests {
    ...
    #[test]
    fn description_template_tracks_renamed_parameters() {
        use crate::types::template_renderer::TemplateRenderer;
        use crate::types::tool::ToolKind;
        use crate::types::tool_metadata::ToolMetadata;

        let renderer = TemplateRenderer::new(
            HashMap::from([(ToolKind::Execute, "run_command".to_string())]),
            HashMap::from([(
                ToolKind::Execute,
                HashMap::from([
                    ("command".to_string(), "script".to_string()),
                    ("timeout".to_string(), "deadline_ms".to_string()),
                ]),
            )]),
        );
        ...
        assert!(
            rendered.contains("optional deadline_ms in milliseconds"),
        ...
        assert!(
            ...
            "stale parameter/tool-name literals must not remain:\n{rendered}"
        );
    }

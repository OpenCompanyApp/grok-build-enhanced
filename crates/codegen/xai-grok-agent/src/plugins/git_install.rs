...
/// into the SHA slot so the verified clone path is used.
...
/// Install a plugin source and discover plugins.
...
/// Validate and normalize URL plus optional ref/SHA for pre-trust clone paths.
...
pub fn update_repo(
    ...
    match &repo.kind {
        ...
        InstallKind::Git {
            ...
            // Check if pinned
            if let Some(r) = git_ref {
                // A full commit object ID or version tag is already a no-op update.
...
mod tests {
    ...
    #[test]
    fn git_operand_validators_reject_unsafe_operands() {
        for url in [
        ...
        for git_ref in ["main", "feature/topic", "refs/tags/v1.2.3"] {
    ...
    #[test]
    fn supplied_sha_requires_full_sha1_or_sha256_hex() {
    ...
    #[test]
    fn install_rejects_invalid_operands_before_install_dir_creation() {
        let root = tempfile::tempdir().unwrap();
        let install_dir = root.path().join("installed-plugins");
        let registry = InstallRegistry::empty(install_dir.clone());
    ...
    #[test]
    fn sha_git_argv_terminates_options_before_operands() {
        assert_eq!(
    ...
    fn install_from_source_gates_and_hoists_sha_pins() {
        ...
        assert!(!registry.install_dir().exists());
        ...
        let result = install_from_source(&pinned_via_ref, &registry, true)
            .expect("a full-SHA ref must use the verified pin path");
        assert_eq!(result.commit.as_deref(), Some(sha.as_str()));
        match result.kind {
            InstallKind::Git { git_ref, .. } => {
                assert_eq!(git_ref.as_deref(), Some(sha.as_str()));
            }
            InstallKind::Local { .. } => panic!("expected Git install"),
        }
    }

    #[test]
    fn update_repo_refuses_mutable_ref_when_policy_requires_sha() {
        let repo = InstalledRepo {
            kind: InstallKind::Git {
                url: "https://example.com/repo.git".into(),
                git_ref: Some("main".into()),

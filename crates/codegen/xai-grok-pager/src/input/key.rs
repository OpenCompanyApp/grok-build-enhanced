...
mod tests {
    ...
    #[test]
    fn row_walk_reads_tab_encodings_and_wraps() {
        ...
        for (code, modifiers) in [
            ...
            assert_eq!(walk(code, modifiers), Some(RowWalk::Backward));
        }
        assert_eq!(walk(KeyCode::Tab, KeyModifiers::CONTROL), None);

        assert_eq!(RowWalk::Forward.step(0, 3), 1);
        assert_eq!(RowWalk::Forward.step(2, 3), 0);
        assert_eq!(RowWalk::Backward.step(0, 3), 2);

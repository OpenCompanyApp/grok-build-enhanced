            ...
            tasks.spawn(async move {
                ...
                match acp_send(request, &tx).await {
                    Ok(resp) => {
                        ...
                        let result_val = wrapper.get("result").cloned().unwrap_or(wrapper);
                        match serde_json::from_value::<crate::views::rewind::RewindResponse>(
                            result_val,
                        ) {
                            Ok(response) => TaskResult::RewindPreviewComplete {
                                agent_id,
                                response,
                                target_prompt_index,
                                mode,
                            },
                            Err(error) => TaskResult::RewindPreviewFailed {
                                agent_id,
                                error: format!("invalid response: {error}"),
                            },
                        }
                    }
                    Err(error) => TaskResult::RewindPreviewFailed {
...
fn session_info_fields(
    ...
    if let Some(profile) = xai_grok_sandbox::profile_name() {
        let network = if xai_grok_sandbox::should_restrict_child_network() {
            " (network: restricted)"
        } else {
            ""
        };
        push("Sandbox", format!("{profile}{network}"), true);
    }

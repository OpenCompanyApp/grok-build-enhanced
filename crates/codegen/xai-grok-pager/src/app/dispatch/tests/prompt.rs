...
/// in up-arrow history with its `! ` prefix. It starts session creation, then waits for the drain.
#[test]
...
fn local_drain_holds_while_server_row_queued() {
    ...
    assert!(
        ...
        "local drain must hold while the server owns the next turn, got {:?}",
        drain.effects
    );
    assert_eq!(
    ...
    let drain = maybe_drain_queue(agent);
    assert!(
        matches!(drain.effects.as_slice(), [Effect::SendPrompt { .. }]),
        "a running-only shared queue must not hold the local drain, got {:?}",
        drain.effects
    );
}

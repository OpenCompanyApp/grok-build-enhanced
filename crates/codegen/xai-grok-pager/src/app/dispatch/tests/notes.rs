...
#[test]
fn newer_minimal_btw_request_suppresses_stale_response() {
    let mut app = test_app_with_agent();
    app.screen_mode = crate::app::ScreenMode::Minimal;
    let id = AgentId(0);
    let old_request = send_minimal_btw(&mut app, "old question");
    let current_request = send_minimal_btw(&mut app, "current question");

    dispatch(
        Action::TaskComplete(TaskResult::BtwResponse {
            agent_id: id,
            result: Ok("stale answer".into()),
            minimal_request_id: Some(old_request),
        }),
        &mut app,
    );
    assert!(matches!(
    ...
    dispatch(
        Action::TaskComplete(TaskResult::BtwResponse {
            agent_id: id,
            result: Ok("current answer".into()),
            minimal_request_id: Some(current_request),
        }),
        &mut app,
    );
    assert!(matches!(
...
fn minimal_btw_requests_stay_independent_across_two_agents() {
    ...
    let first_request = send_minimal_btw(&mut app, "first");

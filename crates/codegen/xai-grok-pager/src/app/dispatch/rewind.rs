...
fn enter_executing(
    ...
    vec![Effect::RewindExecute {
        ...
        session_id,
        target_prompt_index: target,
        // The UI no longer exposes file-restoring rewind modes. Keep the
        // legacy effect field only for wire compatibility and always send the
        // safe conversation-only operation.
...
pub(super) fn handle_rewind_preview_complete(
    ...
    response: crate::views::rewind::RewindResponse,
    target_prompt_index: usize,
    _mode: crate::views::rewind::RewindMode,
) -> Vec<Effect> {
    let Some(agent) = app.agents.get_mut(&agent_id) else {
        return vec![];
    };
    if response.error.is_some() && response.clean_files.is_empty() && response.conflicts.is_empty()
    {
        ...
        let anchor = agent
            .rewind_state
            .as_ref()
            .map(|state| state.anchor_entry_idx)
            .unwrap_or(0);
        let draft = agent
            .rewind_state
            .take()
        ...
        agent.rewind_state = Some(RewindState {
            phase: RewindPhase::Error { message },
            anchor_entry_idx: anchor,
            stashed_draft: draft,
            selected_prompt_index: None,
        });
    ...
    let prompt_preview = agent
        ...
        .and_then(|points| {
            points
                .iter()
                .find(|point| point.prompt_index == target_prompt_index)
        })
        .and_then(|point| point.prompt_preview.clone());
    let draft = agent
        .rewind_state
        .take()
        .and_then(|state| state.stashed_draft);

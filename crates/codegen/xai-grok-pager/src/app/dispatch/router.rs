    ...
    let effects = match action {
        ...
        Action::SwitchModel { model_id, effort } => {
            ...
            let Some(session_id) = agent.session.session_id.clone() else {
                ...
                let unchanged = previous_model.as_ref() == Some(&model_id)
                    && previous_effort == resolved_effort;

                // Keep the switch for the authoritative session catalog once
                // the session exists, but update the pre-session status bar
                // and /model picker immediately.

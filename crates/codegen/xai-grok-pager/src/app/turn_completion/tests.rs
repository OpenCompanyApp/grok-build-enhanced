...
// ── EndLine markers + between-turns status window ──
...
#[test]
fn real_end_marker_stays_plain_and_tracks_running_work() {
    let mut agent = running_driver("p1");
    ...
    assert!(
        ...
        "running work opens the no-wake fallback window"
    );
}
...
#[test]
fn plain_marker_never_mutates_with_background_counts() {
    let mut agent = running_driver("p1");
    insert_bg_task(&mut agent, "bg-1", false);
    push_turn_terminal_marker(
        &mut agent,
    ...
    assert_eq!(
        ...
        "Worked for 2.0s",
        "status-row count changes must never rewrite the terminal marker"
    );
}

...
impl AppView {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        ...
        let model_id = match &target {
            ...
            VoiceTarget::DashboardDispatch => self
                .dashboard
                .as_ref()
                .and_then(|dashboard| dashboard.models.current_model_id_str())
    ...
    /// [`WELCOME_TOAST_DURATION`]. Reconnect success copy cannot hide a
    /// still-current leader version mismatch banner.
            ...
            if view_area.height > 0 {
                match *active_view {
                    ...
                    ActiveView::Agent(id) => {
                        ...
                        let overlay_can_cycle = position.is_some_and(|(_, count)| count > 1);

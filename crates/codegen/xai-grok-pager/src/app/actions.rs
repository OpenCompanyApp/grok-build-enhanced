...
pub enum Action {
    ...
    /// Open the freeform feedback pane (bare `/feedback`).
    OpenFeedbackPane,
    /// Submit feedback text (inline or from the pane).
    SendFeedback(String),
...
pub enum Effect {
    ...
    /// title (and `/rename` manual-ness) plus last-turn dashboard summary.
    HydrateSessionMetaFromDisk {
        agent_id: AgentId,
        session_id: acp::SessionId,
        cwd: std::path::PathBuf,
        /// Summary generation captured at enqueue; a later live update/rewind
        /// prevents this disk result from overwriting current state.
...
pub enum TaskResult {
    ...
    /// Local `summary.json` display metadata read for

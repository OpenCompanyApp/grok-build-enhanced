            ...
            ActiveModal::SessionPicker {
                ...
                match outcome {
                    ...
                    PickerOutcome::SubmitQuery => {
                        // Free-text load is deliberately restricted to a UUID
                        // session id. Own it before clearing the modal because
                        // `state` borrows `active_modal`.
                        let load_id =
                            crate::views::session_picker::session_id_for_direct_load(state.query())
                                .map(str::to_owned);
                        if let Some(session_id) = load_id {
                            self.active_modal = None;
                            InputOutcome::Action(Action::LoadSession(session_id, None, false))
                        } else {
                            InputOutcome::Unchanged
                        }
                    }

...
/// markerless through `finish_wake_turn` in the ACP handler. `event == None`
/// (bash turns, rate-limit / re-auth UX that replaces the marker) flushes the
/// held hooks as the legacy standalone lifecycle block so failures stay visible.
///
...
///
/// Markers remain plain; the persistent turn-status row owns live background
/// counts. The pager separately remembers whether work existed at turn end so
/// no-wake completions can emit their chronological fallback status line.

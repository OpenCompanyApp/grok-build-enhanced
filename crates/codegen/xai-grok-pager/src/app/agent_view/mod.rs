...
pub struct AgentView {
    ...
    /// A real turn ended while background work was still running, so no-wake
    /// completions landing between turns re-emit a fresh work-only status line
    /// after their chip. Wake-bound completions do not: their live count stays
    /// in the persistent status row and their synthetic turns close markerless.
    /// Cleared when a real turn starts and on every replay-window entry.

...
pub enum PromptInputMode {
    ...
    Bash,
    /// Feedback mode (`~` prefix): Enter sends `Action::SendFeedback`.
    Feedback,
...
impl PromptInputMode {
    pub fn accent_color(self, theme: &Theme) -> Option<ratatui::style::Color> {
        match self {
            ...
            PromptInputMode::Feedback => Some(theme.accent_model),
...
pub struct AgentView {
    ...
    /// Wake prompts whose terminals landed; late deltas must not revive them.
    pub(crate) finished_wake_prompts: HashSet<String>,
    /// The wake turn currently streaming, if any.
    ...
    /// A real turn ended while background work remained, allowing no-wake
    /// completions between turns to refresh the work-only status line.
    ...
    /// Clickable ▲ under the pinned prompt header; jumps to response top.
    ...
    /// Whether the dashboard overlay can cycle among multiple agents. Kept
    /// beside `in_dashboard_overlay` so shortcut hints match the visible chips.
    ...
    /// Present only while the permission card is in pattern-edit focus.
    ...
    /// Ultra-short outcome/finding from the most recent successful turn,
    /// preferred over a message preview in an idle dashboard row.
    pub last_turn_summary: Option<String>,
    /// Bumped on every live summary mutation so an in-flight disk hydrate
    /// cannot resurrect stale pre-rewind state.
...
fn translate_local_submit(
    ...
    match kind {
        ...
        LocalQuestionKind::Feedback => unreachable!("handled before option selection"),
    }
}

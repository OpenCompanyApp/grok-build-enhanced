...
impl AgentView {
    ...
    /// below the user's message, so the withheld "Worked for …" marker must
    /// never render under it (it would read as the turn
    ...
    /// [`crate::views::turn_status::Watchers`]. Workflow child sessions are
    /// coalesced into their owning workflow instead of being double-counted as
    ...
    pub(crate) fn current_end_work(&self) -> crate::scrollback::blocks::EndWork {
        ...
        crate::scrollback::blocks::EndWork {
            running_commands: watchers.commands,
            running_monitors: watchers.monitors,
            running_subagents: watchers.subagents + watchers.workflows,
        }
    }

    /// Shared tail of every real turn-end marker push. Markers stay plain;
    /// live background counts belong to the persistent turn-status row. The
    /// separate announce bit only keeps the no-wake completion fallback
    /// chronological between turns.
    pub(crate) fn push_end_marker_block(
        ...
        let end_work_announced = self.current_end_work().nonzero().is_some();
        let folded = self.scrollback.fold_completion_into_tail_parked_marker(
            &event,
            &stop_hooks,
            prompt_id.as_deref(),
        );
        ...
        self.end_work_announced = end_work_announced;
    }
    ...
    /// (`will_wake` stamped by the shell) skip this line because the persistent
    /// status row carries the fresh counts while the wake turn closes markerless.

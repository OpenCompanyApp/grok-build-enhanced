...
pub struct UsageInfoModalState {
    ...
    /// Provider-owned plan/allowance text for isolated subscription providers.
    /// Kept separate from xAI's `CreditBalance` so one provider can never be
    /// represented by another provider's billing state.
...
fn usage_limit_lines(
    ...
    if let Some(provider_usage) = &state.provider_usage_text {
        for row in provider_usage.lines() {

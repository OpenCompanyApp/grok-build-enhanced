...
mod tests {
    ...
    #[test]
    fn idle_local_agent_prefers_last_turn_summary() {
        let mut agent = make_idle_agent_with_model(Some("grok-4.5"));
        agent.last_turn_summary = Some("Completed the parser fix".into());
        agent
            .scrollback
            .push_block(crate::scrollback::block::RenderBlock::agent_message(
                "Older response preview",
            ));
        let row = top_level_row(AgentId(0), &agent, false, false, None);
        assert_eq!(
            row.secondary_line.as_deref(),
            Some("Completed the parser fix")
        );
    }

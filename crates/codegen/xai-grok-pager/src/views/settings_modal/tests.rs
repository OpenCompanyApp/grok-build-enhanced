...
fn every_dynamic_enum_setting_has_action_for_string_arm() {
    ...
    for meta in reg.all() {
        ...
        match meta.key {
            ...
            "theme" => {
                assert!(
                    matches!(nonempty_action, Some(Action::SetTheme(ref value)) if value == "Test Model"),
                    "theme canonical must produce SetTheme with the same runtime value, got {nonempty_action:?}",
                );
            }
            "auto_dark_theme" => {
                assert!(
                    matches!(nonempty_action, Some(Action::SetAutoDarkTheme(ref value)) if value == "Test Model"),
                    "auto_dark_theme canonical must produce SetAutoDarkTheme with the same runtime value, got {nonempty_action:?}",
                );
            }
            "auto_light_theme" => {
                assert!(
                    matches!(nonempty_action, Some(Action::SetAutoLightTheme(ref value)) if value == "Test Model"),
                    "auto_light_theme canonical must produce SetAutoLightTheme with the same runtime value, got {nonempty_action:?}",
                );
            }
            other => panic!(
...
fn picker_original_canonical(state: &SettingsModalState) -> String {
    match state.mode() {
        SettingsModalMode::PickingEnum { original_value, .. } => match original_value {
            ...
            other => panic!("picker original value must be enum-like, got {other:?}"),
        },
...
fn click_settings_breadcrumb_collapses_picker_to_browse() {
    ...
    let mut s = enter_picker_for("theme");
    let original = picker_original_canonical(&s);
    let mut buf = Buffer::empty(area);
    render_settings_modal(&mut buf, area, &mut s, false, None);
    let rect = s
        .settings_breadcrumb_rect
    ...
    // For preview-supporting dynamic themes, the breadcrumb click reverts
    // the runtime original (including terminal-native and Warp selections).
    match outcome {
...
fn click_settings_breadcrumb_after_nav_reverts_to_original() {
    ...
    // advance.
    let orig_canonical_owned = picker_original_canonical(&s);
    let advanced_idx = match s.mode() {
...
fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
    ...
    match outcome {
        SettingsKeyOutcome::ActionPair(
            ...
            assert_eq!(
                orig, original,
                "PreviewTheme revert must carry the runtime original canonical",
            );
        }
        other => {
...
fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
    ...
    let theme_kind = crate::theme::Theme::current_kind();
    let raw_theme = match theme_kind {
    ...
    if !matches!(
        ...
        crate::theme::ThemeKind::TerminalNative
            | crate::theme::ThemeKind::WarpSync
            | crate::theme::ThemeKind::WarpCustom
    ) {
        assert_ne!(
            raw_theme.bg_visual, raw_theme.bg_highlight,
            "built-in theme tokens bg_visual + bg_highlight must be distinct so the preview \

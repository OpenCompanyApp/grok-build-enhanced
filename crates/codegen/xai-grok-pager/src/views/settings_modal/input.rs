...
fn handle_picking_enum(state: &mut SettingsModalState, key: &KeyEvent) -> SettingsKeyOutcome {
    ...
    match key.code {
        ...
        KeyCode::Esc => {
            // Revert preview and return to Browse. Static enums carry a
            // borrowed canonical; runtime-backed dynamic enums carry an owned
            // String. Both must restore the live preview before closing.
            state.transition_to_browse();
        ...
        KeyCode::Char('d') if key.modifiers.is_empty() => {
            ...
            if supports_preview && let Some(revert) = revert {
...
pub(super) fn set_picker_idx(
    ...
    // Preview both static and runtime-backed enum catalogs. The owned lookup
    // preserves discovered Warp theme IDs instead of silently limiting preview
    // to compile-time canonicals.
    if supports_preview
        && let Some(new_canonical) = picker_choice_at_owned(state, setting_key, new_idx)

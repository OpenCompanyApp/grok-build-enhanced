...
- If the policy fetch fails or never completes, emission starts once the
  attempt is exhausted, and in all cases no later than 30 seconds after startup.

A fleet policy that arrives afterwards can only tighten the local settings:
it may disable the stream or force content gates off, but cannot enable
anything the local configuration did not. If the collector receives nothing,
run `grok --debug` and inspect the `external otel:` lines.
...
`startup.total` measures process start to a usable session, once per process.
`startup.phase_duration` covers `load_config`, `managed_policy`, `bootstrap`,
`model_catalog`, `spawn_worker`, `leader_connect`, `acp_initialize`, and
`eager_auth`; use its `outcome` when calculating percentiles. `stuck_in` names
the phase that timed out. `auth_mode` is `personal`, `team`, `deployment`, or
`unknown`.

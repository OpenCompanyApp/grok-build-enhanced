...
TUI. Both relaunch the pager on the same conversation **without changing the
saved default**. The `--minimal` and `--fullscreen` CLI flags are likewise
session-scoped. Set `[ui].screen_mode` in `config.toml` or use the Settings UI
to change how future plain `grok` launches open.

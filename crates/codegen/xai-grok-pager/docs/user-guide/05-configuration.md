...
screen_mode = "fullscreen"             # saved default: "minimal" or "fullscreen"; change here
                                       # or in Settings (CLI/slash switches are session-scoped)
...
The `screen_mode` setting under `[ui]` is the **saved render-mode default** for
plain `grok` launches. Change it in `config.toml` or from `/settings` →
**Default screen mode**.
...
The `--minimal` and `--fullscreen` flags, plus `/minimal` and `/fullscreen`,
override the saved default for the current session without modifying
`config.toml`. `screen_mode` takes precedence over the legacy `[terminal]
minimal` key in `pager.toml`; delete it to restore the legacy behavior.

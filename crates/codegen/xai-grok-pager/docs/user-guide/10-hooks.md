...
`SubagentEnd` is accepted as an alias for `SubagentStop`. `PreToolUse` can block a tool call, while `Stop` and `SubagentStop` can keep an agent turn active; every other event is passive.

...
| [Z.AI GLM Coding Plan](zai-glm-coding-plan-integration-research.md) | **Implemented, experimental; extended GLM 5.2 live matrix qualified 2026-07-19** | Runtime `zai_coding_plan` with isolated `zai_coding_plan_api_key` credentials under `zai::coding-plan::global`, authenticated dynamic models, Coding Plan Chat Completions, plan usage, Search/Reader/Zread MCP, and opt-in pinned Vision MCP. Entitled headless/ACP inference, repeated-prefix caching, quota, hosted MCP, Zread, Vision, attachment routing, invalid-key rejection, and SSRF policy paths are live-qualified; interactive, live-subagent, longer stress, and forced rate/quota/expiry/account-change conditions remain. It never falls through to Z.AI Open Platform or BigModel China endpoints. |
